...
@section('content')
            ...
            <section class="col">
                ...
                <table class="table">
                    ...
                    @foreach (App\ControlEventEquipment::where('control_event_id', $controlEvent->id)->get() as $coEvEquip)
                        @php
                            $conEquip = App\ControlEquipment::where(
                                'equipment_id',
                                $coEvEquip->Equipment->id,
                            )->first();
                        @endphp
                        <tr>
    ...
    @if (!$requirement->is_external)
        ...
        <p class="mb-3">{{ __('Die Anforderung') }} {{ $requirement->an_label }} {{ $requirement->an_name }}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            ...
                                            ><strong>{{ __('nicht bestanden') }}</strong></span>
                                        @endif
                                    </td>
                                </tr>
                            </table>
                            ...
                            <table class="table">
                                ...
                                @foreach (\App\ControlEventDataset::where('control_event_item_id', $ceitem->id)->orderBy('id')->get() as $datasetItem)
                                    <tr>
                                        ...
                                        <td style="text-align: right;">
                                            {{ $datasetItem->valueString($datasetItem->control_event_dataset_read) }}</td>
                                        <td style="text-align: right;">
                                            {{ $datasetItem->valueString(
                                                $datasetItem->AciDataSet->data_point_value - $datasetItem->control_event_dataset_read,
                                            ) }}
                                        </td>
                                        <td style="text-align: right;">{{ $datasetItem->AciDataSet->makeTolString() }}</td>
                                        <td style="text-align: right;">
