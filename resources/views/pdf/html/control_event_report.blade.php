...
@section('content')
        ...
        <tr>
            ...
            <td align="right">
                <h1>{{ __('Prüfbericht') }} {{ $reportNo }}<br>
                    <span style="font-size: small; margin: 0; padding: 0;">{{ __('Prüfung vom:') }}
                        {{ $controlEvent->control_event_date }}</span>
                </h1>
    ...
    >
        ...
        <tr>
            <th style="font-size: small; padding-top:5px; padding-bottom: 5px; font-weight: bold;">
                {{ __('Inventarnummer') }}</th>
            <th style="font-size: small; padding-top:5px; padding-bottom: 5px; font-weight: bold;">
                {{ __('Seriennummer') }}</th>
        </tr>
        <tr>
    ...
    <p></p>
    <p>{!! __('Die nächste Prüfung wurde auf den <strong>:dueDate</strong> gesetzt.', [
        'dueDate' => $controlEvent->control_event_next_due_date,
    ]) !!}</p>
    <p></p>
    <p></p>
    @if (!$controlStep->aci_execution)
        ...
        >
            ...
            <tr>
                <td><span
                            style="font-size: 9pt;"
                    >{{ __('Prüfer') }}</span><br>{{ $controlEvent->control_event_controller_name ?? '-' }}
                </td>
                <td><span
                            style="font-size: 9pt;"
                    >{{ __('Leitung') }}</span><br>{{ $controlEvent->control_event_supervisor_name ?? '-' }}
                </td>
            </tr>
        </table>
    @else
    ...
    @if (App\ControlEventEquipment::where('control_event_id', $controlEvent->id)->count() > 0)
        ...
        >
            <thead>
            <tr>
                <th style="font-size: 11px; font-weight: bold; border-bottom: 1px solid #777777;">{{ __('Gerät') }}
                </th>
                <th style="font-size: 11px; font-weight: bold; border-bottom: 1px solid #777777;">
                    {{ __('Seriennummer') }}</th>
                <th style="font-size: 11px; font-weight: bold; border-bottom: 1px solid #777777;">
                    {{ __('Letzte Prüfung') }}</th>
                <th style="font-size: 11px; font-weight: bold; border-bottom: 1px solid #777777;">
                    {{ __('Nächste Prüfung') }}</th>
            </tr>
            </thead>
            <tbody>
    ...
    @if (!$controlStep->Anforderung->is_external)
        ...
        <p>{{ __('Die Anforderung') }} <em>{{ $requirement->an_label }} {{ $requirement->an_name }}</em>
            {{ $requirementitems->count() > 1 ? __('umfasst folgende Prüfungen:') : __('umfasst folgende Prüfung:') }}
        </p>

        @foreach ($requirementitems as $aci)
            @php
                $ceitem = App\ControlEventItem::withTrashed()
                    ->where([['control_item_aci', $aci->id], ['control_event_id', $controlEvent->id]])
                    ->first();
            @endphp
            @if ($ceitem)
                ...
                >
                    <tr>
                        <td><span
                            ...
                            >{{ __('Aufgabe / Ziel') }}:</span><br>{!! nl2br($aci->aci_name) !!}<br><br><span
                                    style="font-size: 9px; font-weight:
                        bold;"
                            >{{ __('Beschreibung der Prüfung') }}:</span><br>{!! nl2br($aci->aci_task) !!}<br>
                            ...
                            @elseif($aci->aci_value_target_mode === 'dp')
                                ...
                                >
                                    ...
                                    @foreach (\App\ControlEventDataset::where('control_event_item_id', $ceitem->id)->orderBy('id')->get() as $datasetItem)
                                        <tr>
                                            <td style="text-align: right;">
                                                {{ $datasetItem->valueString($datasetItem->AciDataSet->data_point_value) }}
                                            </td>
                                            <td style="text-align: right;">
                                                {{ $datasetItem->valueString($datasetItem->control_event_dataset_read) }}
                                            </td>
                                            <td style="text-align: right;">
                                                {{ $datasetItem->valueString(
                                            ...
                                            <td style="text-align: right;">
                                                @if ($datasetItem->control_event_dataset_pass)
