    ...
    <li class="nav-item dropdown {{ Request::routeIs('event.*')  ? ' active ' : ' ' }}">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarEquipmentEventDropdown">
            ...
            @foreach (\App\EquipmentEvent::where('read','<>',NULL)->take(5)->latest()->get() as $equipMen)
                <a class="dropdown-item" href="{{ route('event.show',$equipMen) }}">{{ $equipMen->created_at }}</a>
            @endforeach
            <div class="dropdown-divider"></div>
            <a class="dropdown-item" href="{{ route('event.index') }}">{{__('Übersicht')}}</a>
            <a class="dropdown-item" href="{{ route('event.create') }}">{{__('Neu')}}</a>
        </div>
    </li>
