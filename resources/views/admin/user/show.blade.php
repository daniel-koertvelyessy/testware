...
@section('breadcrumbs')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{__('Benutzer')}}</li>
        </ol>
    </nav>
@endsection
...
@section('content')
    <div class="container">
        <form action="{{ route('user.update',$user) }}" method="post" class="mb-4">
            ...
            <div class="row">
                <div class="col">
                    <x-rtextfield id="name" label="{{__('Benutzername')}}" value="{{ $user->name }}" />
                    <x-emailfield id="email" label="{{__('E-Mail Adresse')}}" value="{{ $user->email }}" />
                    <x-textfield id="username" label="{{__('Anmeldename')}}" value="{{ $user->username }}" />
                </div>
            </div>
            <div class="row mb-4">
                <div class="col-md-3">
                    <x-datepicker id="created_at" label="{{__('Erstellt')}}" value="{{ $user->created_at }}" />
                </div>
                <div class="col-md-3">
                    <x-datepicker id="updated_at" label="{{__('Aktualisiert')}}" value="{{ $user->updated_at }}" />
                </div>
                <div class="col-md-3">
                    <x-datepicker id="deleted_at" label="{{__('Gelöscht')}}" value="{{ $user->deleted_at }}" />
                </div>
                <div class="col-md-3">
                    <x-selectfield id="locale" label="{{__('Sprache einstellen')}}">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="custom-control custom-switch ml-3">
                        ...
                        <label class="custom-control-label" for="setUserDisplaySimpleView">{{__('Vereinfachte Anzeige von Formularen')}}</label>
                    </div>

                    <div class="custom-control custom-switch ml-3">
                        <input class="custom-control-input" type="checkbox" id="setUserDisplayHelperText" name="setUserDisplayHelperText">
                        <label class="custom-control-label" for="setUserDisplayHelperText">{{__('Hilfetexte anzeigen')}}</label>
                    </div>
                </div>
            </div>
            @if(Auth::user()->id === $user->id)
        ...
        <div class="row mt-5">
            <div class="col">
                ...
                <form action="{{ route('user.resetPassword') }}">
                    <x-rtextfield id="newPassword" label="{{__('Neues Passwort')}}"/>
                    @if(Auth::user()->id === $user->id)
