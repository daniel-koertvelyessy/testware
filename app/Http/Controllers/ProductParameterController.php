...
class ProductParameterController extends Controller
{
    ...
    {
        $countEquipmentUopdated = [];
        foreach (Equipment::where('produkt_id', $parameter->produkt_id)->get() as $equipment) {
            $countEquipmentUopdated[] = ($method === 'add') ?
                (new EquipmentParam)->storeParameter($parameter->toArray(), $equipment->id) :
