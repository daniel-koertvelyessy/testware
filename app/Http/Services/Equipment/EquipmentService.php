...
class EquipmentService {
    ...
    {
        return Equipment::where('eq_serien_nr',
                                'ILIKE',
                                '%'.strtolower($term).'%')
    ...
    public function getUpcomingControlItems(Equipment $equipment)
    {
        ...
        return ControlEquipment::join('anforderungs',
                                      'control_equipment.anforderung_id',
                                      '=',
                                      'anforderungs.id')
            ->select('control_equipment.id',
                     'anforderungs.id AS anforderungs_id',
                     'equipment_id',
                     'anforderungs.is_initial_test',
                     'anforderungs.an_name',
                     'control_equipment.qe_control_date_due')
            ->where('equipment_id',
                    $equipment->id)
            ->where('is_initial_test',
                    false)
    ...
    public function getOneTimeControlItems(Equipment $equipment)
    {
        ...
        return ProduktAnforderung::leftJoin('anforderungs',
                                            'produkt_anforderungs.anforderung_id',
                                            '=',
                                            'anforderungs.id')
            ->where('produkt_id',
                    $equipment->Produkt->id)->get();
    ...
    public function checkUpdatedFields(Request $request, Equipment $oldEquipment): array
    {
        $feld = '';
        $flag = false;
        $feld .= __(':user führte folgende Änderungen durch',
                    ['user' => Auth::user()->username]).': <ul>';
    ...
    {
        foreach (ProductQualifiedUser::where('produkt_id',
                                             $request->produkt_id)->get() as $productQualifiedUser) {
            (new EquipmentQualifiedUser)->addEquipment($productQualifiedUser,
                                                       $equipment->id);
        }
    }
    ...
    {
        foreach (ProductInstructedUser::where('produkt_id',
                                              $request->produkt_id)->get() as $productInstructedUser) {
            (new EquipmentInstruction)->addEquipment($productInstructedUser,
                                                     $equipment->id);
        }
    }
    ...
    public function transferProductParameters(Request $request, Equipment $equipment)
    {
        if (isset($request->pp_id) && count($request->pp_id) > 0) {
            for ($i = 0; $i < count($request->pp_id); $i++) {
                (new EquipmentParam)->addEquipment(
                    $request->pp_id[$i],
                    $request->ep_value[$i],
                    $equipment->id);
            }
    ...
    {
        $userList = [];
        foreach (EquipmentQualifiedUser::select('user_id')->where('equipment_id',
                                                                  $equipment->id)->get() as $user) {
            $userList[] = ['user_id' => $user->user_id];
        }

        foreach (ProductQualifiedUser::select('user_id')->where('produkt_id',
                                                                $equipment->produkt->id)->get() as $user) {
            $userList[] = ['user_id' => $user->user_id];
        }
        return $userList;
    }
    ...
    {
        return ControlEquipment::onlyTrashed()->where('equipment_id',
                                                      $equipment->id)->count() > 0;
    }

    public function getRecentExecutedControls(Equipment $equipment)
    {
        return ControlEquipment::where('equipment_id',
                                       $equipment->id)->take(3)->latest()->onlyTrashed()->get();
    }
