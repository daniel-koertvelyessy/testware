...
class RaumSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $room = factory(App\Room::class, 14)->make()->each(function ($rom) use ($gebaeude) {
            ...
            $rom->save();
            factory(App\Storage::class)->create(['storage_uid'         => $rom->storage_id,
                                                 'storage_label'       => $rom->r_label,
                                                 'storage_object_type' => 'rooms'
            ]);
        });
        ...
        $stellplatz = factory(App\Stellplatz::class, 48)->make()->each(function ($sp) use ($room) {
            ...
            $sp->save();
            factory(App\Storage::class)->create(['storage_uid'         => $sp->storage_id,
                                                 'storage_label'       => $sp->sp_label,
                                                 'storage_object_type' => 'stellplatzs'
            ]);
        });
    }
}
