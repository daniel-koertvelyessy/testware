...
class RaumSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $room = factory(App\Room::class, 14)->make()->each(function ($rom) use ($gebaeude) {
            ...
            $rom->save();
            factory(App\Standort::class)->create(['std_id' => $rom->standort_id, 'std_kurzel' => $rom->r_label, 'std_objekt_typ' => 'rooms']);
        });

        $stellplatz = factory(App\Stellplatz::class, 48)->make()->each(function ($sp) use ($room) {
            $sp->room_id = $room->random()->id;
            $sp->save();
            factory(App\Standort::class)->create(['std_id' => $sp->standort_id, 'std_kurzel' => $sp->sp_label, 'std_objekt_typ' => 'stellplatzs']);
        });
    }
}
